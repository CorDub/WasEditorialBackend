...
import { addBook } from "../../../routes/admin/books/addBook.js";
import {
  createAuthor,
  createBook,
  createTestDB,
...
describe("adding a valid book", () => {
  ...
  it("should create a new book in the database", async() => {
    addedBook = await prisma.book.findUnique({
      where: {
        title_mainAuthor: {
          title: "Yep this is a new book",
          mainAuthor: author1.id
        }
      }
    })
  ...
  it("should create a new inventory for Bodega Was in the database with the given quantity", async() => {
    ...
    const derived = getInventoryDerived(addedInventory[0]);
    ...
    expect(derived.disponibles).toBe(1000);
    expect(derived.impressionInicial).toBe(1000);
  })
})
...
describe("adding a duplicate book", async() => {
  ...
  beforeAll(async() => {
    previousBook = await prisma.book.findUnique({
      where: {
        title_mainAuthor: {
          title: "Yep this is a new book",
          mainAuthor: author1.id
