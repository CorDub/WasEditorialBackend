...
import { updateSale } from "../../../routes/admin/sales/updateSale.js";
...
describe(`updating a sale with valid parameters`, async() => {
  ...
  it(`should update the sale with the correct data`, async() => {
    ...
    expect(updatedSale.dateStr).toBe("2025-09-04");
  })
...
describe(`updating the date of a valid sale for a book with multiple authors`, async() => {
  ...
  beforeAll(async() => {
    ...
    impression = await createImpression(prisma, book1.id, {quantity: 1000});
    ...
    sale1 = await createSale(prisma, inventory1.id, [oldPayment1.id, oldPayment2.id, oldPayment4.id, oldPayment3.id], {quantity: 100, dateStr: "2025-11-02"})
...
describe(`updating a sale with a larger quantity than what's remaining`, async() => {
  ...
  let updatedSale, updatedInventory, updatedInventoryDerived;
  let author;
  let category;
  let book;
  let impression;
  ...
  beforeAll(async() => {
    ...
    category = await createCategory(prisma);
    book = await createBook(prisma, [author.id]);
    impression = await createImpression(prisma, book.id, {quantity: 200})
    payment = await createPayment(prisma, author.id, "2025-11");
  ...
  it(`shouldn't let you update the sale if the new quantity is higher than available in inventory`, async() => {
    ...
    updatedInventoryDerived = getInventoryDerived(updatedInventory)
    expect(updatedSale.quantity).toBe(100);
    expect(updatedInventoryDerived.disponibles).toBe(100);
  })
})
...
describe(`updating a deleted sale`, async() => {
  ...
  beforeAll(async() => {
    ...
    book = await createBook(prisma, [author.id]);
    payment = await createPayment(prisma, author.id, "2025-11")
    impression = await createImpression(prisma, book.id, {quantity: 100})
    newBookstore100 = await createBookstore(prisma);
    newInventory100 = await createInventory(prisma, book.id, newBookstore100.id, {initial: 3000, current: 100, isDeleted: true})
...
is either paid or solicited`, async() => {
  ...
  beforeAll(async() => {
    ...
    sale = await createSale(prisma, inventory.id, [payment2.id], {dateStr: "2025-07-20"})
  ...
  it(`should reassign the date to the nearest available payment (payment4)`, async() => {
    ...
    expect(reassignedSale.dateStr).toEqual("2025-06-20")
