...
import { sendInvoice } from "../../../routes/author/commissions/sendInvoice.js";
import {
  createCategory,
  createAuthor,
  createBook,
...
describe(`sending invoice but the payment is already solicited`, () => {
  let mockReq, mockRes, jsonRes, updatedPayment, mailerSpy, mute;
  let category1;
  let author;
  let book1, book2;
  ...
  beforeAll(async() => {
    ...
    inventory1 = await createInventory(prisma, book1.id, bookstore1.id, {initial: 100, current: 70})
    inventory2 = await createInventory(prisma, book2.id, bookstore2.id, {initial: 100, current: 90})
    payment1 = await createPayment(prisma, author.id, "2025-10", {status: "solicited"})
    payment2 = await createPayment(prisma, author.id, "2025-09")
    payment3 = await createPayment(prisma, author.id, "2023-10")
    sale1 = await createSale(prisma, inventory1.id, [payment1.id], {quantity: 10, dateStr: "2025-10-02"})
    sale2 = await createSale(prisma, inventory2.id, [payment1.id], {quantity: 10, dateStr: "2025-10-02"})
...
describe(`sending invoice but the payment is already paid`, () => {
  let mockReq, mockRes, jsonRes, updatedPayment, mailerSpy, mute;
  let category1;
  let author;
  let book1, book2;
  ...
  beforeAll(async() => {
    ...
    inventory1 = await createInventory(prisma, book1.id, bookstore1.id, {initial: 100, current: 70})
    inventory2 = await createInventory(prisma, book2.id, bookstore2.id, {initial: 100, current: 90})
    payment1 = await createPayment(prisma, author.id, "2025-10", {status: "paid"})
    payment2 = await createPayment(prisma, author.id, "2025-09")
    payment3 = await createPayment(prisma, author.id, "2023-10")
    sale1 = await createSale(prisma, inventory1.id, [payment1.id], {quantity: 10, dateStr: "2025-10-02"})
    sale2 = await createSale(prisma, inventory2.id, [payment1.id], {quantity: 10, dateStr: "2025-10-02"})
...
describe(`sending invoice but the payment doesn't exist`, () => {
  let mockReq, mockRes, jsonRes, updatedPayment, mailerSpy, mute;
  let category1;
  let author;
  let book1, book2;
  ...
  beforeAll(async() => {
    ...
    payment2 = await createPayment(prisma, author.id, "2025-09")
    payment3 = await createPayment(prisma, author.id, "2023-10")
    sale1 = await createSale(prisma, inventory1.id, [payment2.id], {quantity: 10, dateStr: "2025-10-02"})
    sale2 = await createSale(prisma, inventory2.id, [payment2.id], {quantity: 10, dateStr: "2025-10-02"})
    deletedSale = await createSale(prisma, inventory1.id, [payment2.id], {quantity: 10, isDeleted: true, dateStr: "2025-10-02"})
    sale3 = await createSale(prisma, inventory1.id, [payment2.id], {quantity: 10, dateStr: "2025-09-02"})
    sale4 = await createSale(prisma, inventory1.id, [payment3.id], {quantity: 10, dateStr: "2023-10-02"})
    kindleSale1 = await createKindleSale(prisma, book1.id, [payment2.id], {quantityEbook: 5, quantityPod: 5, datePayStr: "2025-10-02", regalias: 100})
    kindleSale2 = await createKindleSale(prisma, book2.id, [payment2.id], {quantityEbook: 5, quantityPod: 5, datePayStr: "2025-09-02", regalias: 100})
    kindleSale3 = await createKindleSale(prisma, book1.id, [payment3.id], {quantityEbook: 5, quantityPod: 5, datePayStr: "2023-10-02", regalias: 100})
    deletedKindleSale = await createKindleSale(prisma, book1.id, [payment2.id], {quantityEbook: 5, quantityPod: 5, datePayStr: "2025-10-02", regalias: 100, isDeleted: true})
    cost1 = await createCost(prisma, payment2.id, book1.id, {amoutn: 100})
    cost2 = await createCost(prisma, payment2.id, book2.id, {amoutn: 100})
    cost3 = await createCost(prisma, payment2.id, book1.id, {amoutn: 100, isDeleted: true})
