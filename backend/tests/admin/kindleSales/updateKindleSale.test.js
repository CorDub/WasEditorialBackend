...
import { updateKindleSale } from "../../../routes/admin/kindleSales/updateKindleSale.js";
import {
  createCategory,
  createAuthor,
  createBook,
...
describe(`updating Kindle sale with valid parameters`, async() => {
  ...
  beforeAll(async() => {
    ...
    newKindleSale = await createKindleSale(prisma, newBook.id, [newPayment.id, newPayment2.id], {quantityEbook: 10, quantityPod: 10, datePayStr: "2025-06-02", regalias: 100})
    deletedKindleSale = await createKindleSale(prisma, newBook.id, [newPayment.id, newPayment2.id], {quantityEbook: 10, quantityPod: 10, datePayStr: "2025-06-02", regalias: 100, isDeleted: true})
    ...
    mockReq = {
      ...
      body: {
        quantityEbook: 100,
        quantityPod: 100,
        dateCutStr: "2025-09-13",
        datePayStr: "2025-11-13",
        regalias: 121000.5
      },
      prisma: prisma
    }
...
describe(`updating a kindleSale date for a book with multiple authors`, async() => {
  ...
  beforeAll(async() => {
    ...
    newKindleSale = await createKindleSale(prisma, newBook.id, [newPayment.id, newPayment2.id, newPayment3.id, newPayment4.id], {quantityEbook: 10, quantityPod: 10, datePayStr: "2025-11-02", regalias: 100})
    deletedKindleSale = await createKindleSale(prisma, newBook.id, [newPayment.id, newPayment2.id], {quantityEbook: 10, quantityPod: 10, datePayStr: "2025-11-02", regalias: 100, isDeleted: true})
    ...
    mockReq = {
      ...
      body: {
        quantityEbook: 100,
        quantityPod: 100,
        dateCutStr: "2025-08-13",
        datePayStr: "2025-10-13",
        regalias: 121000.5
      },
      prisma: prisma
    }
...
(deleted, paid or solicited)`, async() => {
  ...
  beforeAll(async() => {
    ...
    kindleSale = await createKindleSale(prisma, book.id, [payment2.id, payment5.id, payment8.id], {datePayStr: "2025-07-04"})
    ...
    mockReq = {
      ...
      body: {
        quantityEbook: 10,
        quantityPod: 10,
        dateCutStr: "2025-04-04",
        datePayStr: "2025-06-04",
        regalias: 100
      },
      prisma: prisma
    }
